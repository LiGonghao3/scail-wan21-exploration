...
print("窗口提醒：核心扩散模型引擎已就绪！")
# 获取当前脚本所在的根目录
root_dir = os.path.dirname(os.path.abspath(__file__))
if root_dir not in sys.path:
    sys.path.insert(0, root_dir)

# 强制测试导入，如果这里报错，说明路径还是不对
try:
    import sgm.models.wan_vae
    print("窗口提醒：基础模块加载成功！")
except ImportError:
    print("窗口提醒：基础模块依然缺失，请检查文件夹名是否为 sgm")
import math
import argparse
import json
...
if __name__ == '__main__':
    ...
    #     if "sigma_sampler_config" in args.model_config.loss_fn_config.params.keys() and hasattr(
    ...
    print(f"窗口提醒：正在加载配置文件 {args.base[0]} ...")
    with open(args.base[0], 'r', encoding='utf-8') as f:
        full_config = OmegaConf.create(yaml.safe_load(f))

    # 注入模型架构定义
    args.model_config = full_config.model
    # 注入运行参数（如 load 路径、batch_size 等）
    for k, v in full_config.args.items():
        setattr(args, k, v)

    # print(f"窗口提醒：模型架构已设定（{args.model_config.network_config.params.score_model_config.params.num_layers}层）")
