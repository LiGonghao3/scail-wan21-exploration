...
def get_args(args_list=None, parser=None):
    ...
    try:
        ...
        # 定义几个假零件，让参数解析器别报错
        def mock_func(parser):
            return parser

        def mock_init(*args, **kwargs):
            return None

        deepspeed.add_config_arguments = mock_func  # <--- 修复你现在的报错
        deepspeed.init_inference = mock_init

        print("窗口提醒：已补全 DeepSpeed 缺失零件，强行推进程序。")
...
def initialize_distributed(args):
    ...
    print("窗口提醒：单卡运行，跳过 torch.distributed 初始化")
